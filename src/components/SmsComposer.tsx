...
export default function SmsComposer({ schoolId, classes }: { schoolId: string; classes: any[] }) {
  ...
  return (
    <div className="bg-white p-6 rounded-lg shadow-sm border border-slate-200">
      ...
      <div className="mb-4">
        <label htmlFor="sms-target" className="block text-sm font-medium text-slate-700 mb-1">Select Target Class</label>
        <select
          id="sms-target"
        ...
        >
          ...
          {classes.map((cls) => {
            // Display "ParentName - ArmName" if parentId exists
            const label = cls.parentClass 
      ...
      >
        {isSending ? "Sending..." : <><Send size={18} /> Send Bulk SMS</>}
      </button>
    </div>
  );
}